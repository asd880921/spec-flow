...
| 流程 | SA→SD→plan→build→verify (clean 由使用者依情境自由調用) | brainstorm→plan→TDD→review | constitution→specify→plan→tasks→implement |
...
> **依賴程式碼知識圖譜查找**：分析既有專案時，SA / SD 階段透過程式碼知識圖譜查找相關現況（受影響模組、資料流、既有實作），讓設計與實際 codebase 對齊。全新專案則不需要。工作流本身不綁定特定產品，可使用任何能在既有 codebase 中查找模組、流程與資料流的等效工具；設定方式見 [INSTALL.md](./INSTALL.md)。
...
| `codebase-design-doc-cleaner` | 交付清洗（自由調用） | 移除來源標記與待釐清事項，另存乾淨交付版 `_cleaned.md`（原檔不動）；非固定流程步驟，由你自行決定何時清洗 |
...
flowchart TD
    ...
    VERIFY --> DONE([交付])
    CLEAN["codebase-design-doc-cleaner<br/>想要乾淨對外版時自行調用（可多次）<br/>另存 -sd_cleaned.md，原檔不動"]
    SD -. 任何時間想產出交付版 .-> CLEAN
```
...
flowchart TD
    ...
    OVER -. 驗收依據 .-> OVER2
    OVER2 --> DONE([交付])
    CLEAN["codebase-design-doc-cleaner<br/>母  +  各子，想要乾淨對外版時逐份自行調用（可多次）<br/>各自另存 _cleaned.md，原檔不動"]
    OVER -. 任何時間想產出交付版 .-> CLEAN
```
...
flowchart TD
    ...
    VERIFY --> DONE([交付])
    CLEAN["codebase-design-doc-cleaner<br/>想要乾淨對外版時自行調用（可多次）<br/>另存 -dev_cleaned.md，原檔不動"]
    DDOC -. 任何時間想產出交付版 .-> CLEAN
```
...
- **你（人）固定要介入**：① 選完整 / 輕量、（完整時）再選單一 / 拆分；② 每個 commit 人工 review + 手動 `git commit` 並回告 AI 才標記完成；③ 拆分時各模組與母文件的 verifier 由你個別調用。
- **交付清洗是隨時可調用的工具，不是固定的最後一關**：文件常常先發佈初版、開發中又多次修訂，因此 `codebase-design-doc-cleaner` 可能在過程中被多次使用。任何時候你想要一份移除內部註記、可對外交付的乾淨版本，就自行調用它清洗當下的文件；它不綁定在流程末端，spec-flow 也不會主動引導或替你調用。清洗會**另存一份 `<原檔名>_cleaned.md`、原始文件保持不動**，所以你能在原檔上繼續修訂、之後再重新清洗（重清會覆蓋同名的 `_cleaned.md`）。
